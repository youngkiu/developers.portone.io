...
<table>
  ...
  <tr>
    <td>
      ...
      partner.**buseinssRegistrationNumber** string
  ...
  <tr>
    <td>
      ...
      partner.**account** dicitionary

      파트너 정산계좌 정보. Account Attributes
    </td>
  ...
  <tr>
    <td>
      ...
      partner.**status** dicitionary
  
      파트너 상태. Partner Status Attributes
    </td>
  </tr>
  <tr>
    <td>

      partner.**defaultContractId** string
...
정산금액. 최종으로 정산되어야 하는 금액이다. 주문 정산(orderTransfer) 일경우 주문금액에서 수수료 및 할인분담금을 차감한 금액이며
수동 정산(manual) 일경우 수동 정산금액(amount) 이다.
...
`paymentInfo[paymentId]` 를 통해 포트원에 등록된 결제금액만을 기준으로 정산객체를 생성할 경우 필요없습니다.
하지만

1. 할인이 들어간 주문이어서 주문금액 = 할인금액 + 결제금액 인경우
2. 단일 주문에 대해서 여러개의 파트너 별 정산객체를 생성할 경우, 주문금액 분할을 해야할 경우
3. 포트원을 통한 결제가 아닌 외부 결제건일 경우
